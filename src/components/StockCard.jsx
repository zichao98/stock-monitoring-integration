...
export default function StockCard({ stock, currentPrice, history, signal, alerts, onToggleAlert, onSelect, isActive }) {
  const prevPrice = history.length > 1 ? history[history.length - 2].rate : null
  const change = percentChange(currentPrice || 0, prevPrice)
  const isUp = change >= 0

  const sparkData = history.slice(-30).map(h => ({ rate: h.rate }))

  const buyAlertHit = alerts?.buyTarget && currentPrice && currentPrice <= alerts.buyTarget
  const sellAlertHit = alerts?.sellTarget && currentPrice && currentPrice >= alerts.sellTarget
  const hasAlert = buyAlertHit || sellAlertHit

  const marketTag = stock.market === 'TW' ? 'TWSE' : 'US'
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-medium">{marketTag}</span>
          </div>
          <p className="text-xs text-muted-foreground truncate max-w-[180px]">{stock.label}</p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="flex items-end justify-between mb-3">
        <div>
          {currentPrice != null ? (
            <div className="flex items-center gap-2">
              <span className="text-3xl font-bold tabular-nums">{currentPrice.toFixed(2)}</span>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {stock.symbol} @ {currentPrice?.toFixed(2) || '---'}
          </p>
        </div>
